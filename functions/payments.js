...
exports.createPaymentIntent = onCall({ cors: true, invoker: 'public' }, async (request) => {
...
exports.createCheckoutSession = onCall({ cors: true, invoker: 'public' }, async (request) => {
  const uid = request.auth?.uid;
  if (!uid) throw new HttpsError('unauthenticated', 'Login required');
...
exports.webhooksStripeDev = onRequest({ cors: true, invoker: 'public' }, async (req, res) => {
  const stripe = getStripe();
  ...
  console.log(
    'rawBody?',
    req.rawBody ? (Buffer.isBuffer(req.rawBody) ? 'buffer' : typeof req.rawBody) : 'none',
  );
